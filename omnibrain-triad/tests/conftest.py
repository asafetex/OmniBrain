        ...
        json.dumps(
            {
                ...
                "levels": {
                    "L1": {
                        "gate_required": False,
                        "pregate_optional": False,
                        "reviewers": [],
                        "decision_rule": "not_required",
                    },
                    "L2": {
                        "gate_required": False,
                        "pregate_optional": True,
                        "reviewers": ["codex"],
                        "decision_rule": "any_reject",
                    },
                    "L3": {
                        "gate_required": True,
                        "pregate_optional": True,
                        "reviewers": ["codex", "gemini"],
                        "decision_rule": "both_approve",
                    },
                },
                "intents": {
                    "generic": {"keywords": [], "graph_nodes": []},
                    "spark_sql_join": {
                        "keywords": ["join", "explode"],
                        "graph_nodes": ["joins.md"],
                    },
                },
            }
        ),
...
def git_repo(tmp_path: Path) -> Path:
    ...
    subprocess.run(["git", "config", "user.email", "test@test.com"], cwd=repo, capture_output=True, text=True)
    subprocess.run(["git", "config", "user.name", "Test"], cwd=repo, capture_output=True, text=True)
